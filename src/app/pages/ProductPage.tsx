...
export function ProductPage({
  ...
  const [activeTab, setActiveTab] = useState<
    "description" | "additional-info" | "reviews" | "faq"
  >("description");
  ...
  const hasVariations =
    (product.variations?.length ?? 0) > 0 && variationAttributes.length > 0;
  ...
  useEffect(() => {
    ...
    const first =
      product.variations?.find((v) => v.inStock) ?? product.variations?.[0];
    if (!first) return;
  ...
  const displayInStock = selectedVariation
  ...
  const galleryImages = useMemo(() => {
    const all = [...(product.images ?? []), displayImage].filter(
      Boolean,
    ) as string[];
  ...
  const faqs = [
    {
      q: `What are the benefits of ${product.name}?`,
      a: product.benefits.join(". ") + ".",
    },
    { q: "How should I take this supplement?", a: product.dosage },
    {
      q: "Are there any side effects?",
      a: "This supplement is generally well-tolerated. Consult your healthcare provider before starting any new supplement regimen, especially if you are pregnant, nursing, or taking medications.",
    },
    {
      q: "Is this product third-party tested?",
      a: "Yes, all our products undergo rigorous third-party testing for purity, potency, and safety. We are GMP certified and committed to the highest quality standards.",
    },
  ];
      ...
      <div className="grid lg:grid-cols-2 gap-8 mb-12">
        ...
        <div>
          <div className="relative rounded-xl overflow-hidden bg-secondary aspect-square">
            ...
            {discount > 0 && (
              <span
                className="absolute top-4 right-4 bg-accent text-white px-3 py-1 rounded-full text-sm"
            ...
            {product.labels.length > 0 && (
              <div className="absolute top-4 left-4 flex flex-col gap-1">
                {product.labels.map((label) => (
                  <span
                    key={label}
                    className={`text-xs px-2 py-1 rounded-full ${label === "Best Seller" ? "bg-primary text-white" : label === "Sale" ? "bg-accent text-white" : label === "New" ? "bg-blue-500 text-white" : "bg-green-100 text-green-700"}`}
                  >
                    {label}
                  </span>
                ))}
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-3">
            <div className="flex">
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
                  className={`w-5 h-5 ${i < Math.floor(displayRating) ? "fill-yellow-400 text-yellow-400" : "text-gray-200"}`}
                />
              ))}
            </div>
          ...
          <div className="mt-4">
            {displayInStock ? (
              <span className="text-sm text-primary flex items-center gap-1">
                ...
                In Stock
                {displayStockCount && displayStockCount <= 5 && (
              ...
              {Object.keys(selectedOptions).length > 0 &&
                !selectedVariation && (
                  <p className="text-sm text-destructive">
                    This combination is not available. Please choose different
                    options.
                  </p>
                )}
            </div>
          ...
          <div className="flex items-center gap-3 mt-6">
            ...
            <button
              ...
              disabled={
                !displayInStock || (hasVariations && !selectedVariation)
              }
          ...
          <div className="grid grid-cols-3 gap-3 mt-6">
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <Truck className="w-4 h-4 text-primary shrink-0" /> Free Shipping
              $49+
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <RotateCcw className="w-4 h-4 text-primary shrink-0" /> 30-Day
              Returns
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <Shield className="w-4 h-4 text-primary shrink-0" /> Quality
              Guaranteed
            </div>
          </div>
        </div>
          ...
          {activeTab === "faq" && (
            <div className="max-w-2xl space-y-4">
              ...
              {faqs.map((faq, i) => (
                ...
                >
                  <summary
                    className="p-4 cursor-pointer text-sm list-none flex items-center justify-between"
